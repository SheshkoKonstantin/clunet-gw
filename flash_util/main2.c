...
unsigned char raddress=255;
char wbuf[RBUF_SZ+1];
unsigned char wbuf_c=0;
...
int transfer_byte(int from, int to, int is_control) {
    ...
    */
    ...
    if (c=='\r') {
    	    for (int i=0;i<rbuf_p;++i) printf ("%02x ",rbuf[i]);
	    printf("\r\n");
	if (rbuf[0]=='P' && rbuf_p>=4) {
	    unsigned char addr=rbuf[1];
	    unsigned char comm=rbuf[2];
	    unsigned char size=rbuf[3];
	    printf("POCKET! FROM %u COM %u SIZE %u .\r\n", addr, comm, size);
	    //for (int i=0;i<size;++i) printf ("%02x ",rbuf[4+i]);

	    
	    // БУТЛОДЕР ГОТОВ? 1 ШАГ
	    if ((comm==2)&&(size==1)&&(rbuf[4])==0) {
		printf ("BOOTLOADER\r\n");
		//unsigned char buf[6]={'f', addr, 8, 2, 1, '\r'};
...
*/
    ...
    if ((rbuf_p<RBUF_SZ)&&(c!='\r')) { rbuf[rbuf_p]=c; ++rbuf_p; }
